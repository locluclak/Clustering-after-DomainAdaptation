...
def test_statistic(X_vec, Xt, ns, nt, d, n_clusters, Sigma, labels_all_obs, members_all_obs,return_sign=False):
...
def parametric(model, X, a, b,np_wdgrl, n_clusters, c1, c2, c1_obs, c2_obs, signobs, zmin = -20, zmax = 20, log=None, seed=None):
    global device
    n, d = X.shape
    z =  zmin
    zmax = zmax
    ...
    with tqdm(total=total_steps, desc=f"Seed {seed}") as pbar:
        while z < zmax:
            ...
            sign_z = test_statistic(vec(Xdeltaz),Xdeltaz[ns:], ns, nt, d, n_clusters, Sigma=None, labels_all_obs=labels_all_obs, members_all_obs=members_all_obs,return_sign=True )
...
def run(mu_s, mu_t, K, device,_=None):
    ...
    try:
        a, b, etaTX, etaT_Sigma_eta, c1, c2, c1_obs, c2_obs, sign = test_statistic(X_vec, Xt, ns, nt, d, K, Sigma, labels_all_obs, members_all_obs).values()
