...
def parametric(model, X, a, b, n_clusters, c1, c2, c1_obs, c2_obs, zmin = -20, zmax = 20, log=None, seed=None):
    ...
    with tqdm(total=total_steps, desc=f"Seed {seed}") as pbar:
        while z < zmax:
            z += stepsize
            # print("z =",z)
            Xdeltaz = (a + b*z).reshape(n, d, order='F')
...
def run(mu_s, mu_t, K, device,_=None):
    ...
    try:
        ...
        b_2d = b.reshape(n, d, order='F')
        final_interval = overconditioning(final_model, X_origin, a_2d, b_2d, K, initial_centroids_obs, labels_all_obs, members_all_obs)
        # st = time.time()
