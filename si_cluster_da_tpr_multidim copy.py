...
def run(mu_s, mu_t, K, device,_=None):
    ...
    try:
        ...
        return None
    check_h1 =np.sum(np.abs(np.mean(mut[c1_obs], axis=1)))/mut.shape[1] - np.sum(np.abs(np.mean(mut[c2_obs], axis=1)))/mut.shape[1]
    print(c1_obs)
    print("mut c1 obs")
    print(*mut[c1_obs], sep = "\n")
    print("mut c2 obs")
    print(*mut[c2_obs], sep = "\n")
    print(mut[c1_obs].shape)
    print(mut[c2_obs].shape)
    print(np.mean(mut[c1_obs], axis = 0))
    print(np.mean(mut[c2_obs], axis = 0))
    check_h1_2 = np.sum(np.abs(np.mean(mut[c1_obs], axis = 0) - np.mean(mut[c2_obs], axis = 0)))/mut.shape[1]
    print(check_h1_2)
